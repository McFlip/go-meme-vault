...
Given("There are new images in the full image path", () => {
  cy.request("http://localhost:8080/public/img/full/doom_eternal.jpg")
    .its("status")
    .should("equal", 200);
});
...
When("I delete the tag", () => {
  cy.get("#tags").findByRole("button", { name: "X" }).click();
});

Then("I should not see any tags for this meme", () => {
  cy.get("#tags").findByText("first tag").should("not.exist");
});
